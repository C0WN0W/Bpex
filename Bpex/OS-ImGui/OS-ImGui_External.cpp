...
namespace OSImGui
{
    ...
    void  OSImGui_External::Attach(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw OSException("CallBack is empty");
    ...
    void OSImGui_External::AttachByLyric(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw OSException("CallBack is empty");
        if (DestWindowName.empty() && DestWindowClassName.empty())
    ...
    {
		// Kugou Music Lyrics Overlay
        HWND hWnd = FindWindowW(
            L"kugou_ui",
            L"\x684c\x9762\x6b4c\x8bcd\x0020\x002d\x0020\x9177\x72d7\x97f3\x4e50"
        );
        if (!hWnd)
            return false;
